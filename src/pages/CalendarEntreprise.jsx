...
function CalendarEntreprise() {
  ...
  const [endpoint, setEndpoint] = useState("forCalendrier/societe/evenements");


  const navigateToEventPage = (id) => {
    navigate(`/societe/evenements/${id}`);
  };
  ...
  useEffect(() => {
    if (data) {
      // const transformedEvents = transformEventData(data); // Transformer les données
      setEvents(data); // Mettre à jour le state avec les événements transformés
    }
                ...
                <div className="mb-4 sm:flex sm:justify-between sm:items-center">
                  ...
                  <div className="mb-4 sm:mb-0">
                    <h1 className="text-2xl font-bold text-gray-800 md:text-3xl dark:text-gray-100">
                      <span>{`${monthNames[((month % 12) + 12) % 12]} ${year + Math.floor(month / 12)
                        }`}</span>
                    </h1>
                  </div>
                          ...
                          return (
                            ...
                            >
                              <div className="flex flex-col justify-between h-full">
                                ...
                                <button
                                  className={`inline-flex ml-auto w-6 h-6 items-center justify-center text-xs sm:text-sm dark:text-gray-300 font-medium text-center rounded-full hover:bg-violet-100 dark:hover:bg-gray-600 ${isToday(day) &&
...
export default CalendarEntreprise;
