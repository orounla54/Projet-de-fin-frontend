...
function AdminServicesListing({
  ...
  const currentProjects = services.slice(
    indexOfFirstProject,
    indexOfLastProject
  );
  ...
  const handleSelectAll = () => {
    ...
    setIsCheck(!selectAll ? currentProjects.map((Service) => Service.id) : []);
  };
    ...
    try {
      const deleteResults = await Promise.all(
        selectedItems.map(async (id) => {
          try {
            const response = await axios.delete(`${baseUrl}/${element}/${id}`, {
              headers: {
          ...
          <div className="sm:flex sm:justify-between sm:items-center mb-5">
            ...
            <div className="mb-4 sm:mb-0">
              <h1 className="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">
                {element === "categories" ? "Categories evenements" : ""}{" "}
              </h1>
            </div>
...
export default AdminServicesListing;
