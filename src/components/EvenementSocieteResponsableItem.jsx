...
function EvenementSocieteResponsableItem({
  responsable,
  fetchDataDemande,
  event,
  responsableLog,
  ...
  const removeresponsable = async (idResponsable) => {
    ...
    try {
      ...
      await axios.delete(
        `${baseUrl}/responsable/${idResponsable}/societe/evenements/${idEvent}`,
        {
          headers: {
        ...
        <ul className="flex flex-wrap -m-1 sm:justify-end">
          <li className="m-1">
            {parseInt(responsableLog.id) === parseInt(event.idResponsable) ? (
              <button
                onClick={() => {
                  if (event?.idResponsable !== responsable.id) {
                ...
                className={`inline-flex items-center justify-center text-xs font-medium leading-5 rounded-full px-2.5 py-0.5 border shadow-sm transition ${
                  event?.idResponsable === responsable.id
                    ...
                    : "border-red-200 dark:border-red-700/60 hover:border-red-300 dark:hover:border-red-600 bg-white dark:bg-gray-800 text-red-500 dark:text-red-400"
                }`}
              >
                {loading ? (
                  <SpinnerLoading deleteLoading />
                ) : (
                  <>
                    {event?.idResponsable !== responsable.id
                      ? "Retirer"
...
export default EvenementSocieteResponsableItem;
