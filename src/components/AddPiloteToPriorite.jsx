...
function AddPiloteToPriorite({
  ...
  useEffect(() => {
    if (servicesData && rolesData) {
      setServices(servicesData);
      setRoles(rolesData);
      setServiceSelectId(servicesData?.[0]?.id);
    }
  }, [servicesData ,rolesData]);

  // Gestion des changements dans la liste
  const handleSelectChange = (priorite) => {
    const selectedIds = Array.from(priorite.target.selectedOptions, (option) =>
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // Envoyer la requête
      await axios.post(`${baseUrl}/priorites/addToResponsables`, baseDataObject, {
        headers: {
...
export default AddPiloteToPriorite;
