import React from "react";
import Avatar from "react-avatar";

function PostesResponsablesItem({ responsable }) {
  return (
    <li key={responsable.id}>
      <div className="text-xs">
        <div className="flex items-center gap-1">
          <Avatar
            name={`${responsable.nom} ${responsable.prenom}`}
            round={true}
            size="35"
            src={responsable.image}
          />
          <div className="flex flex-col text-xs">
            <span className="font-semibold">
              {responsable?.nom} {responsable?.prenom}
            </span>
            <span>@{responsable?.email}</span>
          </div>
        </div>
        <div>
          <div className="ml-10 flex items-center">
            <span>
              {responsable?.position ? `${responsable.position}, ` : ""}
            </span>
            <span>
              {" "}
              {responsable?.service || "Dans aucun service ou direction"}
            </span>
          </div>
          <span className="ml-10 mt-2 flex">
            {responsable?.poste ? (
              <>
                <svg
                  viewBox="0 0 20 20"
                  fill="currentColor"
                  className="size-4 mr-1"
                >
                  <path
                    fillRule="evenodd"
                    d="M6 3.75A2.75 2.75 0 0 1 8.75 1h2.5A2.75 2.75 0 0 1 14 3.75v.443c.572.055 1.14.122 1.706.2C17.053 4.582 18 5.75 18 7.07v3.469c0 1.126-.694 2.191-1.83 2.54-1.952.599-4.024.921-6.17.921s-4.219-.322-6.17-.921C2.694 12.73 2 11.665 2 10.539V7.07c0-1.321.947-2.489 2.294-2.676A41.047 41.047 0 0 1 6 4.193V3.75Zm6.5 0v.325a41.622 41.622 0 0 0-5 0V3.75c0-.69.56-1.25 1.25-1.25h2.5c.69 0 1.25.56 1.25 1.25ZM10 10a1 1 0 0 0-1 1v.01a1 1 0 0 0 1 1h.01a1 1 0 0 0 1-1V11a1 1 0 0 0-1-1H10Z"
                    clipRule="evenodd"
                  />
                  <path d="M3 15.055v-.684c.126.053.255.1.39.142 2.092.642 4.313.987 6.61.987 2.297 0 4.518-.345 6.61-.987.135-.041.264-.089.39-.142v.684c0 1.347-.985 2.53-2.363 2.686a41.454 41.454 0 0 1-9.274 0C3.985 17.585 3 16.402 3 15.055Z" />
                </svg>
                {responsable.poste}
              </>
            ) : (
              ""
            )}
          </span>
        </div>
      </div>
    </li>
  );
}

export default PostesResponsablesItem;
