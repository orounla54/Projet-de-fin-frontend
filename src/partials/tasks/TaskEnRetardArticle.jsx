...
function TaskEnRetardArticle({ tache }) {
  console.log("#######Tache ReRRRR");
  console.log(tache);
    ...
    if (tache) {
      try {
        ...
        if (tache.responsablesJson) {
          ...
          console.log("Responsables :", tache.responsablesJson);
        } else {
  ...
  } = useGetData(`/taches/ImgsTache/${tache.id}`);
      ...
      >
        <header className="flex items-start mb-2">
          ...
          <div className="flex items-center justify-between w-full gap-2">
            <Link
              to={`/taches/${tache.id}`}
              className="text-xl font-bold leading-snug text-gray-800 dark:text-gray-100"
            >
              {tache && tache.libelle ? (
        ...
        <div className="pl-7">
          <div className="mb-3 mr-16">
            {tache && tache.description ? (
              ...
              <span className="text-xs">Aucune description ajouter...</span>
            )}
          </div>
          {/* images */}
          {imgsTache && imgsTache.length > 0 ? (
            <>
              <div>
                <div className="grid grid-cols-1 gap-2 my-6 xl:grid-cols-3">
                  {imgsTache.slice(0, 3).map((img) => (
                    <>
                      <a className="relative block" href={img.lien}>
                        <img
                          className="w-full rounded-sm h-52"
                          src={img.lien}
                          width="203"
                          height="152"
                          alt={img.libelle}
          ...
          <div className="flex items-center justify-between">
            <ul className="flex flex-wrap">
              ...
              <li className="flex items-center after:block after:content-['·'] last:after:content-[''] after:text-sm after:text-gray-400 dark:after:text-gray-600 after:px-2">
                <a className="p-1 px-2 text-sm font-medium text-center ">
                  {/* Right side */}
              ...
              <li className="flex items-center after:block after:content-['·'] last:after:content-[''] after:text-sm after:text-gray-400 dark:after:text-gray-600 after:px-2">
                <a
                  className="flex items-center text-sm font-medium text-orange-500 hover:text-orange-600 dark:hover:text-orange-400"
                  href="#0"
                >
                  {totalSousTache > 0 && (
                    <>
...
export default TaskEnRetardArticle;
